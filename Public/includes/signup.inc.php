<?php
  ...
  if (isset($_POST["signup-submit"])) {
    ...
    if (empty($username) || empty($email) || empty($password) || empty($passwordRepeat)) {
      ...
      header("Location: ../views/signup.php?error=emptyfields&uid=$username&mail=$email");
      exit();
    }
      ...
      header("Location: ../views/signup.php?error=passwordmalformed&uid=$username&mail=$email");
      exit();
    }
    else if ($password !== $passwordRepeat) {

      header("Location: ../views/signup.php?error=passwordcheck&uid=$username&mail=$email");
      exit();
    }
    else {
      ...
      if ($result === 1062) {
        ...
        header("Location: ../views/signup.php?error=invalidsql");
        exit();
      }
    }
  }
